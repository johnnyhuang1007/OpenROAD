...
bool Replace::initNesterovPlace(int threads)
{
	...
	if (!np_) {
		...
		npVars.FFClusteringMode = FFClusteringMode_;
...
int Replace::doNesterovPlace(int threads, int start_iter)
{
	...
	if(FFClusteringMode_)
	{
		std::cout<<"hello openroad"<<std::endl;
	}
...
void Replace::setFFClusteringMode(bool mode)
{
	FFClusteringMode_ = mode;
}
