                                ...
                                bool propagate)
{
  ...
  if (vertex->isRoot()) {
    ...
    if (propagate)
      iter_->enqueueAdjacentVertices(vertex); //bfs insertion
  }
  else {
    if (network_->isLeaf(pin)) {  //isLeaf means the pin is belong to a simple cell
      if (vertex->isDriver(network_)) {
...
{
  bool observed = false;
  if(drvr_vertex->to_string(this).find("i59/i144") != std::string::npos)
    observed = true;
  bool delay_changed = false;
