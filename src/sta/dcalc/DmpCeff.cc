...
public:
  ...
  virtual void loadDelaySlew(const Pin *load_pin,
			     ...
			     ArcDelay &delay,
			     Slew &slew);
  double ceff() const { return ceff_; }
...
{
  // Effective capacitance used by the most recent DMP evaluation.
  // Valid only after setCeffAlgorithm() has selected an algorithm.
  return dmp_alg_ ? static_cast<float>(dmp_alg_->ceff()) : 0.0F;
}
...
{
  std::string name = "";//network_->pathName(drvr_pin);
  const RiseFall *rf = arc->toEdge()->asRiseFall();
  const LibertyCell *drvr_cell = arc->from()->libertyCell();
  const LibertyLibrary *drvr_library = drvr_cell->libertyLibrary();
  std::string debug_ = "dmp_ceff";
  if(name.find("i55/i460") != std::string::npos)
  {
    std::cout<<name<<std::endl;
  std::cout<<"debug_: "<<debug_<<" DmpCeffDelayCalc::gateDelay() called"<<std::endl;
  std::cout<<"Table Model: "<<arc->gateTableModel(dcalc_ap)<<std::endl;
  std::cout<<"Parasitic: "<<parasitic<<std::endl;
  }
  ...
  if (table_model && parasitic) {
    ...
    if(name.find("i55/i460")!= std::string::npos)
      std::cout<<"c2: "<<c2<<" rpi: "<<rpi<<" c1: "<<c1<<std::endl;
    ...
    if(name.find("i55/i460")!= std::string::npos)
    {
      std::cout<<"in_slew: "<<in_slew1<<std::endl;
      std::cout<<"load_cap: "<<load_cap<<std::endl;
      std::cout<<"gate delay: "<<gate_delay<<" drvr_slew: "<<drvr_slew<<std::endl;
    }
