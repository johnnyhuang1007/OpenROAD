...
public:
  ...
  virtual std::string reportCheckDelay(const Pin *check_pin,
                                       ...
                                       const DcalcAnalysisPt *dcalc_ap,
                                       int digits) = 0;
  // Effective output capacitance (Ceff) used by the most recent
  // gateDelay/reportGateDelay evaluation. The default implementation
  // returns 0; delay calculators that compute an effective
  // capacitance (for example, DMP-based calculators) may override
  // this to export their current Ceff.
  virtual float ceff() const { return 0.0F; }
